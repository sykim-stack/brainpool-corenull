...
export default function HouseDetailPage() {
  ...
  // TopBar 🏠 목적지 — 항상 "나의 마당"(내 house_id 기준), 지금 보고
  // 있는 house와 무관하게 고정. TabBar 패턴과 동일하게 device_id로 조회.
  ...
  useEffect(() => {
    ...
    // 나의 마당 목적지 — 이 house가 내 house든 남의 house든 상관없이,
    // 내 device_id로 찾은 house가 항상 🏠의 목적지다.
    if (key) {
    ...
    <div>
      ...
      <div style={styles.header}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {/* 🏠 — 항상 나의 마당으로. §13 Navigation Context 중 지금
              구현하는 부분만: 광장이 아직 없어서 고정 목적지로 둔다.
              TODO: 광장 생기면 "나의 마당에 있을 때"는 🏛️로 교체 대상. */}
          {myHouseId && (
            <button
              style={styles.iconBtn}
              onClick={() => router.push(`/houses/${myHouseId}/yard`)}
              aria-label="나의 마당"
            >
              🏠
            </button>
