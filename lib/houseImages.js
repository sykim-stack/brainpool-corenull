...
const DEFAULT_HERO_POSITION = { x: 50, y: 50, scale: 1 }

function heroPosition(value) {
  return {
    x: Number.isFinite(Number(value?.x)) ? Number(value.x) : DEFAULT_HERO_POSITION.x,
    y: Number.isFinite(Number(value?.y)) ? Number(value.y) : DEFAULT_HERO_POSITION.y,
    scale: Number.isFinite(Number(value?.scale)) ? Number(value.scale) : DEFAULT_HERO_POSITION.scale,
  }
}
...
export function houseHeroBackground(house, view) {
  if (view === 'living') {
    return {
      ...
      position: heroPosition(house?.living_image_position),
    }
  }
  return {
    imageUrl: house?.yard_image_url || null,
    gradient: DEFAULT_YARD_GRADIENT,
    position: heroPosition(house?.yard_image_position),
  }
}
